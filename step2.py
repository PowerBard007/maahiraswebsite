...
def run_ai_analysis():
    ...
    try:
        ...
        for col in optional_cols:
            if col not in df.columns:
                print(f"      ⚠ Optional column '{col}' not found - some features will be limited")
        
    except FileNotFoundError:
        print(f"      ✗ ERROR: {INPUT_FILE} not found.")
        print(f"      → Please run Phase 1 data generation first.")
        sys.exit(1)
    except ValueError as e:
        print(f"      ✗ ERROR: {e}")
        print(f"      → Check Phase 1 output format.")
        sys.exit(1)
    ...
    if missing_features:
        print(f"      ⚠ Missing features (will use defaults): {missing_features}")
    ...
    model.fit(df_ai)
    print(f"      ✓ Model trained on {len(df_ai)} samples")
    ...
    df_ai['anomaly_score_raw'] = model.decision_function(df_ai[available_features])
    ...
    print(f"      ✓ Detected {len(anomalies)} anomalies out of {len(df)} samples ({100*len(anomalies)/len(df):.1f}%)")
    ...
    print(f"      ✓ Root cause classification complete")
    ...
    if 'Is_Cascading_Failure' in df.columns:
        ...
        print(f"      ✓ Identified {len(primary)} primary failures and {len(cascading)} cascading failures")
    ...
    if not temporal_df.empty:
        print(f"      ✓ Detected {len(temporal_df)} temporal patterns")
        ...
        print(f"      ✓ No significant temporal patterns detected")
    ...
    if len(anomalies) > 0:
        ...
        print(f"\n🔍 ROOT CAUSE ANALYSIS")
        print("-" * 80)
        # Filter anomalies from the updated df that has Root_Cause
        anomalies = df[df['AI_Risk_Assessment'] == 'CRITICAL_RISK']
        ...
        if 'Is_Cascading_Failure' in df.columns:
            ...
            if len(cascading) > 0:
                ...
                print(f"\n💡 RECOMMENDATION: Focus maintenance on these {len(primary)} primary failure points")
                ...
                if len(primary) > 0:
                    ...
                    for idx, row in primary.head(5).iterrows():
                        print(f"  • {row['Asset_ID']:20s} ({row['Asset_Model']:30s}) - {row['Root_Cause']}")
        ...
        if not temporal_df.empty:
            ...
            for idx, row in temporal_df.head(5).iterrows():
                print(f"  • {row['Asset_ID']:20s}: {row['Description']}")
        
        # Top critical assets
        print(f"\n🚨 TOP 10 CRITICAL ASSETS (Most Anomalous)")
        print("-" * 80)
        ...
        if len(df_with_anomalies) > 1:
            ...
            for idx, (feature, corr) in enumerate(correlations_sorted.head(5).items(), 1):
                direction = "↑ Higher" if correlations[feature] > 0 else "↓ Lower"
        ...
        try:
            ...
            print(f"  ✓ Anomaly report saved: {OUTPUT_ANOMALY_FILE}")
            ...
            print(f"  ✓ Full analyzed dataset saved: {full_output}")
        ...
        if critical_count > 0:
            print(f"  🔴 IMMEDIATE (CRITICAL): {critical_count} assets require emergency intervention")
            critical_assets = df[df['Severity'] == 'CRITICAL'].head(3)
            for idx, row in critical_assets.iterrows():
                print(f"     → {row['Asset_ID']}: {row['Root_Cause']}")
        
        if high_count > 0:
            print(f"  🟠 URGENT (HIGH): {high_count} assets need attention within 24 hours")
        
        medium_count = severity_counts.get('MEDIUM', 0)
        if medium_count > 0:
            print(f"  🟡 SCHEDULE (MEDIUM): {medium_count} assets require planned maintenance")
        
        # Security alerts
        security_issues = anomalies[anomalies['Root_Cause'].str.contains('SECURITY', na=False)]
        if len(security_issues) > 0:
            print(f"\n  🔒 SECURITY ALERT: {len(security_issues)} potential cyber threats detected")
    ...
    print(f"\n📂 OUTPUT FILES:")
    print(f"   • {OUTPUT_ANOMALY_FILE} - Critical assets requiring attention")
    print(f"   • {MODEL_FILE} - Trained AI model (reusable)")
    print(f"   • belden_network_logs_analyzed.csv - Complete dataset with AI insights")
    print(f"   • analysis_summary.txt - Executive summary")
    if not temporal_df.empty:
        print(f"   • temporal_patterns.csv - Time-series patterns")
...
def export_for_dashboard(df, anomalies, output_prefix='dashboard_'):
    ...
    try:
        ...
        print(f"✓ Dashboard data exported with prefix: {output_prefix}")
...
if __name__ == "__main__":
    try:
        ...
        print("\n🎉 All Phase 2 processing complete!")
